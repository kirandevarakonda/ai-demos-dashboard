...
if not api_key:
    print(json.dumps({"error": "OPENAI_API_KEY not found in environment variables"}), file=sys.stderr)
    sys.exit(1)

try:
    client = OpenAI(api_key=api_key)
except Exception as e:
    print(json.dumps({"error": f"Failed to initialize OpenAI client: {str(e)}"}), file=sys.stderr)
    sys.exit(1)
...
def generate_keywords(seed_keyword: str) -> List[str]:
    ...
    try:
        ...
        print(json.dumps({"error": f"Error generating keywords: {str(e)}"}), file=sys.stderr)
        sys.exit(1)
